...
static void answer(struct ofono_voicecall *vc, ofono_voicecall_cb_t cb, void *data)
{
	...
	if (qmi_service_send(vd->voice, PARAM_CALL_ID, param,
			...
			ofono_voicecall_cb_t cb, void *data)
{
	...
	if (qmi_service_send(vd->voice, PARAM_CALL_ID, param, end_call_cb,
			cbd, l_free) > 0)
		return;
...
static void stop_cont_dtmf_cb(struct qmi_result *result, void *user_data)
{
	...
	if (qmi_result_set_error(result, &error)) {
		DBG("QMI Error %d", error);
		CALLBACK_WITH_FAILURE(cb, vd->send_dtmf_data);
		return;
	}

	CALLBACK_WITH_SUCCESS(cb, vd->send_dtmf_data);
	l_free(vd->send_dtmf_data);
}
...
static void start_cont_dtmf_cb(struct qmi_result *result, void *user_data)
{
	...
	if (qmi_service_send(vd->voice, QMI_VOICE_STOP_CONTINUOUS_DTMF, param,
			stop_cont_dtmf_cb, vd->send_dtmf_data, NULL) > 0)
		return;
				...
				ofono_voicecall_cb_t cb, void *data)
{
	...
	if (qmi_service_send(vd->voice, QMI_VOICE_START_CONTINUOUS_DTMF, param,
			start_cont_dtmf_cb, vd->send_dtmf_data, NULL) > 0)
		return;
			...
			ofono_voicecall_cb_t cb, void *data)
{
	...
	l_free(vd->send_dtmf_data);
	vd->send_dtmf_data = cb_data_new(cb, data);;
	vd->send_dtmf_data->user = vc;
	vd->send_dtmf_cb = cb;
